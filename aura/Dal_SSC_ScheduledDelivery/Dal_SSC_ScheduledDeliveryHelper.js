...
({
    ...
    getOrdersWithScheduledDeliveries: function(cmp){
        var self = this;
        var listLimit = cmp.get("v.listMaximum");

        // make call to controller to get open items
        var promise = this.doListCallout(cmp, 'c.getScheduledDeliveries', {'listLimit' : listLimit});

        // set callbacks for callout response
        promise.then(function(response){
            self.handleGetOrdersWithScheduledDeliveries(cmp, response);
        }, function(response){
            console.log('Dal_SSC_ScheduledDelivery:getOrdersScheduledDeliveries:Error:', response);
        });
    },

    handleGetOrdersWithScheduledDeliveries: function(cmp, response){
        console.log('handleGetOrdersWithScheduledDeliveries:', response);
