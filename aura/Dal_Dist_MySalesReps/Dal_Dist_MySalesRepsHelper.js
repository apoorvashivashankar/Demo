...
({
    ...
    init: function(cmp){
        ...
        var promise = this.doCallout(cmp, 'c.getMyCustomerCreditAndSalesRep');

        // set callbacks for callout response
        promise.then(function(response){
            self.handleMyTeamSuccess(cmp, response);
            cmp.set('v.loading', false);
        }, function(response){
            console.log('Dal_Dist_MySales:init:error', response);
        });

    },

    /**
     * Request handler for getting my team reps
     * @param cmp
     * @param response
     */
